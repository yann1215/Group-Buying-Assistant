...
这是一个用于金属徽章无盈利拼团的程序，目的是帮助车主核对成员、计算均摊、计算大货。（当前版本：v0.4.0_beta）
...
初次运行程序时，会弹出要求用户输入 key 的窗口。可以使用 wx_key 等工具获取文字 key 之后输入。（**测试版在 wx_decrypt 文件夹内提供了 wx_key。如果使用 wx_key，请确保程序路径为纯英文**，否则 wx_key 会闪退。不需要图像 key。）
